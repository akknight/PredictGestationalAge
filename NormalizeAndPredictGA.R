...
#This file is a slightly modified version of Suppl25 from
#Horvath Genome Biology 2013 14:R115  doi:10.1186/gb-2013-14-10-r115

#Changes made: 
#1) Since the goal is to estimate GA age in weeks, removed the anti.trafo function to transform adult age
#2) Removed warning based on meanMethBySample, since mean methylation of these CpG sites is lower for cord blood and blood spot data
...
if ( sum( selectCpGsClock) < dim(datClock)[[1]]-1 ) {stop("The CpGs listed in column 1 of the input data did not contain the CpGs needed for calculating DNAm GA. Make sure to input cg numbers such as cg00075967.")}
...
dim(datMethClock)
predictedAge=as.numeric(datClock$CoefficientTraining[1]+as.matrix(datMethClock)%*% as.numeric(datClock$CoefficientTraining[-1]))
} # end of if
...
Comment=ifelse ( predictedAge <0, "Negative DNAm GA.", ifelse ( predictedAge >100, "Old DNAm GA (>100 weeks).", rep("",length(predictedAge))))

Comment[is.na(predictedAge)]="Age prediction was not possible. "


if ( sum( selectCpGsClock) < dim(datClock)[[1]]-1 ) {
Comment=rep("ERROR: The CpGs listed in column 1 of the input data did not contain the CpGs needed for calculating DNAm GA. Make sure to input cg numbers such as cg00075967.",length(predictedAge) )}
...
datout=data.frame(SampleID=colnames(dat1)[-1], DNAmGA=predictedAge, Comment, noMissingPerSample,meanMethBySample, minMethBySample, maxMethBySample)
